...
function generateSchema(modelParam) {
	...
	return _.map(models, (model) => {
		const processedSchema = processSchema(model);
...
function fillRelationshipFields(model, processedSchema) {
	_.forOwn(model.relationMappings, (relationEntry, key) => {
		...
		if (Model.HasOneRelation === relationEntry.relation) {
			processedSchema.properties[key] = processSchema(model, relatedClass)
		} else if (Model.HasManyRelation === relationEntry.relation) {
			processedSchema.properties[key] = {
				type: 'array',
				items: processSchema(relatedClass, relatedClass)
			}
		}
	});
}
