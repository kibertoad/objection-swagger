...
function _transformIntoQueryParamSchema(processedSchema) {
	const parameters = [];
...
function saveQueryParamSchema(schemaParam, targetDir, opts = {}) {
	...
	const yamlSchemaContainers = _.map(schemaParam, (schema) => {
		let processedSchema = _processSchema(schema, opts);
		processedSchema = _transformIntoQueryParamSchema(processedSchema);
