...
function generateSchema(modelParam, opts = {}) {
	...
	return models.map((model) => {
		const processedSchema = modelTransformer.transformJsonSchemaFromModel(model, opts);
...
function saveQueryParamSchema(schemaParam, targetDir, opts = {}) {
	...
	const yamlSchemaContainers = schemaParam.map((schema) => {
		...
		const queryParamSchema = queryParamTransformer.transformIntoQueryParamSchema(modelSchema);
