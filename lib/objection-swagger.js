...
function fillRelationshipFields(model, processedSchema, opts) {
	_.forOwn(model.relationMappings, (relationEntry, key) => {
		...
		if (Model.HasOneRelation === relationEntry.relation) {
			processedSchema.properties[key] = resolveSchema(relatedClass, model, opts);
		} else if (Model.HasManyRelation === relationEntry.relation) {
			const items = resolveSchema(relatedClass, model, opts);
...
function resolveSchema(relatedClass, processedClass, opts) {
	if (processedClass !== relatedClass) {
        return opts.useEntityRefs? getRefForModel(relatedClass) : processSchema(relatedClass, opts);
	}
    return {
		type: 'object'
	}
