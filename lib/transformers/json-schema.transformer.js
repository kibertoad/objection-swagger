  ...
  _.forOwn(schema.properties, (value, key) => {
    if (value.private) {
      ...
      if (value.type) {
        if (Array.isArray(value.type)) {
          ...
          if (isFillRequiredList && !_propertyIsNullable(value)) {
            ...
            _requiredFieldSanityCheck(
              key,
              processedSchema.required.includes(key),
      ...
      if (
        ...
        processedSchema.properties[key].type === 'object'
      ) {
...
function _propertyIsNullable(propertyEntry) {
  return (
    Array.isArray(propertyEntry.type) && propertyEntry.type.includes('null')
  );
}
