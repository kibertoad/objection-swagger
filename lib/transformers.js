...
function _resolveModelProperties(modelClass, includeRelationships) {
	const model = _.isString(modelClass) ? require(modelClass) : modelClass;

	if (includeRelationships && model.relationMappings) {
...
function enrichSchemaWithRelationships(schema, relationships, isIncludeParentRelationships, fromModelClass) {
	...
	_.forOwn(relationships, (value, key) => {
			...
			if (value.relation.name === Model.HasOneRelation.name) {
				...
				const modelProperties = value.modelClass !== fromModelClass ? _resolveModelProperties(value.modelClass, true) : { type: 'object' };
				relationshipValue = {
					...modelProperties
				}
			} else if (isIncludeParentRelationships && value.relation.name === Model.BelongsToOneRelation.name) {
				//protect from endless recursion
				const modelProperties = value.modelClass !== fromModelClass ? _resolveModelProperties(value.modelClass, false) : { type: 'object' };
				relationshipValue = {
					...modelProperties
				}
